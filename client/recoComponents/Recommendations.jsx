...
class Recommendations extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      totalSize: props.build[props.activeBuild].size,
      purifyCSSSize: 0,
      minifySize: 0 };
  ...
  handleSelectAll() {
    ...
    if (!this.props.build[1].assets[0].minified) {
      newState.minifySize = this.props.build[1].assets[0].miniSize;
    }
  ...
  handleUnselectAll() {
    const newState = this.state;
    newState.minifySize = 0;
    newState.purifyCSSSize = 0;
    newState.options.map((option) => {
      option.checked = '';
      return option;
    });
